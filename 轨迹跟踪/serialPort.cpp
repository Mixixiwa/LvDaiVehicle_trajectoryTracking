...
{
    std::cout << "[串口发送] 左轮RPM: " << m_aSpeedCommand
        << " 右轮RPM: " << m_bSpeedCommand << std::endl;
    //逆时针为正转，顺时针为反转
    //A电机为左电机
    int a = m_aSpeedCommand / 0.3;
    //B电机为右电机
    ...
    if (a >= 0 && b >= 0)  //判断A，B转速是否大于零
    {
        ...
        buf[10] = (b >> 8) & 0xFF;
        buf[11] = b & 0xFF;
        //判断A电机指令转向与当前A电机转向是否相同
        if (sgn(a) == sgn(m_aSpeedNow))  //A电机相同转向，只需判断B是否需要发送0转速指令
        {
            if (sgn(b) == sgn(m_bSpeedNow))  //B电机转速相同，不需要发送0转速指令
            {
                SendPortMesg(buf, 12);
            }
            else                            //B电机转速不相同，需要先发送0转速指令，在发送目标指令
            {
                SendPortMesg(bufB0, 12);
                SendPortMesg(buf, 12);
            }
        }
        else                               //A电机不同转向，先发送A电机0转速指令，再判断B是否需要发送0转速指令
        {
            SendPortMesg(bufA0, 12);
            if (sgn(b) == sgn(m_bSpeedNow))  //B电机转速相同，不需要发送0转速指令
            {
